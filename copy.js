...
const CustomerPage = () => {
  ...
  const [retailerLocations, setRetailerLocations] = useState({});
  ...
  if (!cid || cid === 'demo-id') return;
  fetch(`http://localhost:5000/api/user/${cid}`)
    .then(res => res.json())
   .then(data => data?.location && setCustomerLocation(data.location));
}, []);

useEffect(() => {
  async function fetchRetailerLocs() {
    const locs = {};
    await Promise.all(products.map(async prod => {
      if (prod.retailerId) {
        const res = await fetch(`http://localhost:5000/api/user/${prod.retailerId}`);
        const data = await res.json();
        if (data?.location) locs[prod.retailerId] = data.location;
      }
    }));
    setRetailerLocations(locs);
  }
  if (products.length) fetchRetailerLocs();
}, [products]);
  ...
  if (customerLocation) {
    filtered = [...filtered].sort((a, b) => {
      const locA = retailerLocations[a.retailerId];
      const locB = retailerLocations[b.retailerId];
      if (!locA) return 1;
      if (!locB) return -1;
      const distA = calculateDistance(customerLocation.lat, customerLocation.lng, locA.lat, locA.lng);
      const distB = calculateDistance(customerLocation.lat, customerLocation.lng, locB.lat, locB.lng);
      return distA - distB;
    });
  }
}
            ...
            filteredProducts.map(prod => (
              ...
              >
                ...
                <div className="flex flex-col flex-1 px-6 pb-4 bg-white dark:bg-gray-800 rounded-b-3xl">
                  ...
                  {customerLocation && retailerLocations[prod.retailerId] && (
                  <div className="mb-1 text-xs text-blue-400">
                  {calculateDistance(
                  customerLocation.lat, customerLocation.lng,
                  retailerLocations[prod.retailerId].lat, retailerLocations[prod.retailerId].lng
                  ).toFixed(1)} km away
                  </div>
                  )}
